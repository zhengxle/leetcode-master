...
leetcode改了测试用例，导致创建vector报错：
```
terminate called after throwing an instance of 'std::length_error'
  what():  cannot create std::vector larger than max_size()
```
主要是(sum+target)/2超过了vector可以创建的最大的size;

需要如下改一下:
```
(sum-y)-y = target
```
其中y是符号改为负号后，所有数据的绝对值之和；进而: 
```
y = (sum-target)/2
```
其他逻辑跟上述逻辑一致即可,代码如下：
...
class Solution:
    def findTargetSumWays(self, nums: List[int], target: int) -> int:
        ...
        if (total_sum - target) % 2 == 1:
            return 0  # 此时没有方案
        target_diff = (total_sum - target) // 2  # 目标和

        # 创建二维动态规划数组，行表示选取的元素数量，列表示累加和
        dp = [[0] * (target_diff + 1) for _ in range(len(nums) + 1)]
        ...
        return dp[len(nums)][target_diff]  # 返回达到目标和的方案数
...
class Solution:
    def findTargetSumWays(self, nums: List[int], target: int) -> int:
        ...
        if (total_sum - target) % 2 == 1:
            return 0  # 此时没有方案
        target_diff = (total_sum - target) // 2  # 目标和
        dp = [0] * (target_diff + 1)  # 创建动态规划数组，初始化为0
        dp[0] = 1  # 当目标和为0时，只有一种方案，即什么都不选
        for num in nums:
            for j in range(target_diff, num - 1, -1):
                dp[j] += dp[j - num]  # 状态转移方程，累加不同选择方式的数量
        return dp[target_diff]  # 返回达到目标和的方案数
